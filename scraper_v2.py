...
def extract_plz_ort(text: str) -> str:
    """
    Extrahiert PLZ + Ortsname aus einem Adressstring.

    Das Format auf rad-net ist: "Straßenname PLZ Ortsname Hallenname"
    z.B. "Raiffeisenplatz 56299 Ochtendung Wernerseck-Halle Route erstellen"

    Strategie:
      1. Suche 5-stellige PLZ im Text.
      2. Nimm das erste Wort direkt danach als Ortsnamen
         (nur Buchstaben, Bindestriche, Schrägstriche – kein Hallenzusatz).
      3. Bereinige "Route erstellen" und ähnliche Junk-Wörter.
    """
    if not text:
        return ""

    # PLZ finden – danach kommt Ortsname (erstes "Wort" mit Buchstaben)
    m = re.search(r"(\d{5})\s+([A-ZÄÖÜa-zäöüß][A-ZÄÖÜa-zäöüß0-9/\-]*(?:\s+[A-ZÄÖÜa-zäöüß][A-ZÄÖÜa-zäöüß\-]*)?)", text)
    if not m:
        return ""
    ...
    # "Route erstellen" und alles danach entfernen
    ort_raw = re.split(r"Route erstellen|route erstellen|\bWeg\b|\bHalle\b|\bPlatz\b|\bStr\b", ort_raw)[0]

    ort = re.sub(r"\s+", " ", ort_raw).strip()
